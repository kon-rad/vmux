...
PHASE 5 SEQUENTIAL: tracks A/B/C are merged. Allowed task IDs are T-025, T-026, T-027 (and they must run in this order — each blocks the next).

1. Find the first task in PRD.md section 7 whose checkbox is "- [ ]" AND whose ID is in {T-025, T-026, T-027}. Call it T-NNN. If no such task exists, append "ALL TASKS COMPLETE" to progress.txt and exit.
